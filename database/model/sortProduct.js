...
module.exports.noFilter = async (req) => {
...
module.exports.withFilter = async (req) => {
    ...
    const result = await db.connection.execute(`select distinct products.* from products, category_product ${filter_sql} order by products.${sortBy} ${sortOrder}`);
    return result[0];
}